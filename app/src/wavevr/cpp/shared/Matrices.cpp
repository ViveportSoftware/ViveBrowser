...
//------------------ static part ------------------------
Matrix4 Matrix4::MakeQuatFromToRotation(const Vector4 &from,const Vector4 &to)
{
    Matrix4 result;
    q_type t_quat;
    q_vec_type a,b;
    a[0] = from.x; a[1] = from.y; a[2] = from.z;
    b[0] = to.x; b[1] = to.y; b[2] = to.z; 
    //make look at quaternion.
    q_from_two_vecs(t_quat,a,b);
    //convert quat type to q mat4
    qogl_matrix_type rstMat;
    q_to_ogl_matrix(rstMat, t_quat);
    result.set(rstMat[ 0],rstMat[ 1],rstMat[ 2],rstMat[ 3],
               rstMat[ 4],rstMat[ 5],rstMat[ 6],rstMat[ 7],
               rstMat[ 8],rstMat[ 9],rstMat[10],rstMat[11],
               rstMat[12],rstMat[13],rstMat[14],rstMat[15]);
    return result;
}

Matrix4 Matrix4::MakeRotationFromQuat(const q_type i_q)
{
    Matrix4 result;
    qogl_matrix_type rstMat;
    q_to_ogl_matrix(rstMat, i_q);
    result.set(rstMat[ 0],rstMat[ 1],rstMat[ 2],rstMat[ 3],
               rstMat[ 4],rstMat[ 5],rstMat[ 6],rstMat[ 7],
               rstMat[ 8],rstMat[ 9],rstMat[10],rstMat[11],
               rstMat[12],rstMat[13],rstMat[14],rstMat[15]);
...
Matrix4::Matrix4(const q_xyz_quat_struct &i_trans)
{
    qogl_matrix_type rstMat;
    q_xyz_quat_to_ogl_matrix(rstMat, &i_trans);
    set(rstMat[ 0],rstMat[ 1],rstMat[ 2],rstMat[ 3],
        rstMat[ 4],rstMat[ 5],rstMat[ 6],rstMat[ 7],
        rstMat[ 8],rstMat[ 9],rstMat[10],rstMat[11],
        rstMat[12],rstMat[13],rstMat[14],rstMat[15]);
}

Matrix4::Matrix4(q_vec_type euler)
{
    q_type quat;
    q_from_euler(quat, euler[Q_Z], euler[Q_Y], euler[Q_X]);
    Matrix4 result;
    qogl_matrix_type rstMat;
    q_to_ogl_matrix(rstMat, quat);
    set(rstMat[ 0],rstMat[ 1],rstMat[ 2],rstMat[ 3],
        rstMat[ 4],rstMat[ 5],rstMat[ 6],rstMat[ 7],
        rstMat[ 8],rstMat[ 9],rstMat[10],rstMat[11],
        rstMat[12],rstMat[13],rstMat[14],rstMat[15]);
...
///////////////////////////////////////////////////////////////////////////////
// Set translation to this matrix by (x, y, z)
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
// Add translation of this matrix by (x, y, z)
///////////////////////////////////////////////////////////////////////////////
Matrix4& Matrix4::addTranslation(const Vector3& v)
{
    return addTranslation(v.x, v.y, v.z);
}

Matrix4& Matrix4::addTranslation(float x, float y, float z)
{
    m[12] += x;
    m[13] += y;
    m[14] += z;
...
void Matrix4::ToQuat(q_type io_quat) const
{
    qogl_matrix_type oglm;
    for(int i = 0; i < 16; i++)
       oglm[i] = static_cast<double>(m[i]);
    q_from_ogl_matrix(io_quat,oglm);
}

void Matrix4::ToEuler(q_vec_type io_euler) const
{
    q_type quat;
    qogl_matrix_type oglm;
    for(int i = 0; i < 16; i++)
       oglm[i] = static_cast<double>(m[i]);
    q_from_ogl_matrix(quat, oglm);
    q_to_euler(io_euler, quat);
}
